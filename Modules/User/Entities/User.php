...
class User extends Authenticatable
{
    ...
    public function getJabatanAttribute(){
        ...
        foreach ($detail as $key => $value) {
            if($value->projectptuser != null){
             $jabatan[$key] = $value->projectptuser->pt_id;
            }
        }
        ...
        foreach ($pt as $key => $value) {
            ...
            foreach ($this->details as $key2 => $value2) {
                if ($value2->mappingperusahaan != null) {
                    # code...
        ...
        foreach ($jabat_pt as $key => $value) {
            if(count($value['jabatan']) != 0){
                for($i=0 ; $i < count($value['jabatan']) ; $i++){
                    $pt = Pt::find($value['pt']);            
                    $jabatan = UserJabatan::find($value['jabatan'][$i]);
                    ...
                    if ( $value['level'][$i] <= 5 ){
                            ...
                            if ( $value['division'][$i] != ""){
                                $division = Division::find($value['division'][$i])->name;
                            }
                    }
    ...
    public function pengajuan(){
        return $this->hasMany("Modules\Spk\Entities\RekananPengajuanIpk","pic_id");
    }
}
