      ...
      <div class="box box-default">
        ...
        <div class="box-body">
          <div class="row">
            ...
            <div class="col-md-6">              
              <form action="{{ url('/')}}/user/update-user" method="post" name="form1" enctype="multipart/form-data">
                  {{ csrf_field() }}                  
                  ...
                  <div class="form-group">
                    <label for="exampleInputEmail1">Password Mobile</label>
                    <input type="password" class="form-control" name="password_mobile" autocomplete="off">
                  </div>
                  <div class="form-group">
                  ...
                  <div class="form-group">
                    ...
                    <img src="{{ url('/')}}/assets/user/{{ $users->id }}/{{ $users->name_file}}" style="width: 300px;">
                  </div>
              ...
              <div class="tab-content">
                    ...
                    <ul class="nav nav-tabs">        
                      @foreach ( $project_pt_user as $key6 => $value6 )   
                        @if ( $value6->project_pts != "" )
                          @if ( $key6 == 0 )     
                          <li  class="active"><a href="#tab_{{ $key6 + 5 }}" data-toggle="tab">{{ $value6->project_pts->project->name }} - {{ $value6->project_pts->pt->name }}</a></li>
                          @else
                          <li><a href="#tab_{{ $key6 + 5 }}" data-toggle="tab">{{ $value6->project_pts->project->name }} - {{ $value6->project_pts->pt->name }}</a></li>
                          @endif
                        @endif
                      @endforeach        
                    </ul>
                        ...
                        @if ( $value6->project_pts != "" )
                          ...
                          <div class="tab-pane {{ $active }}" id="tab_{{ $key6 + 5 }}">
                            ...
                            @foreach ( $value6->user->details as $key7 => $value7 )      
                              {{-- {{$value7->projectptuser}} <br><br>             --}}
                              {{-- @if ( $value6->pt_id == $value7->projectptuser->pt->id ) --}}
                ...
                <div class="tab-pane" id="tab_2">
                  ...
                  <form action="{{ url('/')}}/user/save-detail" method="post">
                    <input type="hidden" class="form-control" name="user_id" id="user_id" value="{{ $users->id }}">
                    {{ csrf_field() }}
                    ...
                    <div class="form-group">
                      ...
                      <button class="btn btn-danger" type="button" id="delete_all">delete all</button>
                      @endif
                    </div>
                  ...
                  <table class="table table-bordered">
                    ...
                    <tbody>
                      ...
                      <tr>
                        ...
                        <td>{{ $value['jabatan'] }}</td>
                        <td><button class="btn btn-danger" onclick="removeuser('{{ $value['pt_id']}}','{{ $users->id}}','{{ $value['jabatan_id']}}','{{$value['project_id']}}')">Delete</button></td>
                      </tr>
                      @endforeach
                    </tbody>
                  </table>
                            ...
                            <select class="form-control select2" required name="project_s">
                              @foreach( $pt as $key7 => $value7 )
                                @foreach ( $value7->project as $key8 => $value8 )
                                  @if ($value8->project != null )
                                    <option value="{{ $value8->id }}">{{ $value7->name}} - {{ $value8->project->name}}</option>
                                  @else
                                  <option value="{{ $value8->id }}">{{ $value7->name}} - +++</option>
                                  @endif
                      ...
                      <tbody>
                        ...
                        <tr>
                          ...
                          <td>
                            ...
                            <button class="btn btn-danger" onclick="deletept('{{ $value->id }}','{{$users->id}}')">Delete</button>
                          </td>
                        </tr>
                        @endif
                        @endforeach
...
<script type="text/javascript">
  ...
  function removeuser(pt_id,user_id,jabatan_id,project_id){
