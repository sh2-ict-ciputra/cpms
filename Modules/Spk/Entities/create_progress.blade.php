                   ...
                   <tbody>
                    @php $nilai = 0; $real_bobot = 0; $total = 0; @endphp
                      @foreach ( $unitprogress as $key2 => $value )
                          @php 
                            $total = $total + ($value->volume * $value->nilai);
                          @endphp
                      @endforeach
                        ...
                        @if ( $value2->spkvo_unit->head_type != "Modules\Spk\Entities\Vo")
                          @if( $value2->volume != 0)
                            <tr style="{{ $arrayEscrow[$value2->itempekerjaan->escrow_id]['style'] }}">                                
                                ...
                                <td>
                                @php
                                $vo_pengurangan = $spk->new_vo->where("tipe",3);
                                $kurang = 0;
                                foreach ($vo_pengurangan as $key => $value) {
                                    # code...
                                    $pengurangan = $value->detail->where('itempekerjaan_id', $value2->itempekerjaan_id)->where('unit_id',$unit->id)->first();
                                ...
                                <!-- {{$kurang}} -->
                                {{$kurang}}
                                {{ number_format((($value2->volume - $kurang) * $value2->nilai)/($total - $nilaitot_kurang) *100,2) }}
                                ...
                                <td> {{ number_format ( $real_bobot_s = ( ( $value2->progresslapangan_percent*100 ) / 100 ) * ($value2->volume * $value2->nilai)/$total *100,2)}}</td> 
                              <!--  <td>
                                  @if ( $value2->mulai_jadwal_date != "" && $value2->selesai_jadwal_date != "" )
                ...
                @foreach($vo->where('tipe',1) as $key => $value3)
                  ...
                  @endforeach
                    <form class="form-horizontal" action="{{ url('/progress/saveschedulevo') }}" method="post" autocomplete="off" name="form-oe" id="form-oe">
                        {{ csrf_field() }}
                      <center><h3>Variation Order {{$key+1}} : {{$value3->no}}</h3></center>
                      ...
                      <table class="table table-bordered">
                        ...
                        <tbody>
                          @php $nilai = 0; $real_bobot = 0; $total = 0; @endphp
                            @foreach ( $value3->detail as $key2 => $value )
                                @php 
                                  $total = $total + ($value->volume * $value->nilai);
                                @endphp
                            @endforeach
                            @foreach ( $value3->detail as $key2 => $value2 )
                                @if( $value2->volume != 0)
                                  <tr style="{{ $arrayEscrow[$value2->itempekerjaan->escrow_id]['style'] }}">                                
                                      <td>
                                        <input type="hidden" value="{{$value3->spk_id}}" name="idspk">
                                        <input type="hidden" value="{{$value2->id}}" name="idvod[{{$key2}}]">
                                        <input type="hidden" name="unitprogress_id_vo_[{{$key2}}]" id="unitprogress_id_vo_[{{$key2}}]" value="{{ $value2->id}}">
                                        {{ $value2->itempekerjaan->name }}<br> {{ $arrayEscrow[$value2->itempekerjaan->escrow_id]['label']  }}
                                      </td> 
                                      <td>{{ number_format(($value2->volume * $value2->nilai)/$total *100,2) }} </td>
                                      <td>
                                        @if ( $value2->mulai_jadwal_date == NULL )
