                        ...
                        <div class="row">
                            <div class="col-md-12">
                                ...
                                <form id="regForm" action="{{ url('/')}}/tenderpurchaserequest/storePenawaran" method="post" name="form1" autocomplete="off" class="form-horizontal">
                                    ...
                                    <div id="divSupplier" class="form-group">
                                        <label class="col-md-3 control-label">Tanggal Barang dibutuhkan</label>
                                        <div class="col-sm-3">
                                            <input type="date" name="date_dibutuhkan" id="date_dibutuhkan" class="form-control" value="" readonly>
                                        </div>
                                    </div>
                                    ...
                                    <div id="divSupplier" class="percentage_dp form-group">
                                    ...
                                    <div class="form-group metode_sisa_pembayaran">
                                    ...
                                    <input type="hidden" name="ppn_rekanan" id="ppn_rekanan" value="">
                                    <div class="form-group">
                                        <div class="col-sm-offset-2 col-sm-10">
                                            </button> <button type="button" id="btn-cicil" class="btn btn-success" style="display: none;"><i class="fa fa-plus"></i> Cicilan</button>
                                        </div>
                                    </div>
                                    ...
                                    <div class="tab-content">
                                        ...
                                        <div id="home" class="tab-pane fade in active">
                                            ...
                                            <table id="table_data" class="table table-striped table-bordered table-hover table-responsive table-checkable order-column nowrap">
                                                <thead style="background: greenyellow;">
                                                    <tr>
                                                        ...
                                                        <th rowspan="2" class="text-center" id="hrga">Harga Satuan (Rp/...)
                                                        </th>
                                                        <th rowspan="2" class="text-center">Total (Rp.)</th>
                                                        <!-- <th rowspan="2" class="text-center" id="hrga">PPN (%)</th> -->
                                                        <th rowspan="2" class="text-center"> Deskripsi (Spesifikasi)</th>
                                                        <th id="cod_head" rowspan="2" class="text-center">COD</th>
                                                    </tr>
                                                    <tr>
                                                        <th>Item</th>
                                                        <th>Brand</th>
                                            ...
                                            <table class="table" id="table_total">
                                                <thead>
                                                    <tr>
                                                        <th class="text-right"><strong>Sub Total (Rp.) :</strong></th>
                                                        <th class="text-right" id="total_keseluruhan_harga">0</th>
                                                        <th></th>
                                                        <th></th>
                                                    </tr>
                                                    <tr>
                                                        <th class="text-right"><strong>PPN (Rp.) :</strong></th>
                                                        <th class="text-right" id="total_ppn">0</th>
                                                        <th></th>
                                                        <th></th>
                                                    </tr>
                                                    <tr>
                                                        <th class="text-right"><strong>Grand Total (Rp.) :</strong></th>
                                        ...
                                        <button type="submit" id="btn-submit" class="btn btn-primary">Simpan
                                    </div>
                                </form>
                                <!-- /.col -->
                            </div>
    ...
    <script type="text/javascript">
        ...
        var _initTable = function() {
            ...
            var _mRender = function(data, type, row) {
                if (type == 'display') {
                    return "<button type='button' class='button-cod btn btn-danger btn-xs'><i class='cod_body fa fa-list'></i> COD</button>";
                }
            ...
            gentable = $('#table_data').DataTable(datatableDefaultOptions)
                .on('click', '.button-cod', function(d) {
                    ...
                    $('.modal-title').text('').append(data.quantity + ' ' + data.satuan_name + ' ' + data
                        .item_penawaran).find('input').remove();
                    $('.modal-title').append('<input type="hidden" name="qty_acuan" id="qty_acuan" value="' +
                        data.quantity + '"><input type="hidden" name="idx" id="idx" value="' + idx + '">');
        ...
        var fnTotalPpn = function() {
            ...
            $('#table_data > tbody > tr').each(function() {
                ...
                var percentage_ppn = parseInt($('#ppn_rekanan').val());
        ...
        $(document).ready(function() {
                    ...
                    success: function(data) {
                        ...
                        if (data.data.length > 0) {
                            ...
                            $(data.data).each(function(i, v) {
                                var strOption = '';
                                if (Object.keys(data.brand_ditawarkan[i])[0] == v
                                    .itemid) {
                                    strOption +=
                                        '<option value="">Semua Brand</option>';
                                    for (var j = 0; j < Object.values(data
                                            .brand_ditawarkan[i])[0].length; j++) {
                                        strOption += '<option value="' + Object
                                            .values(data.brand_ditawarkan[i])[0][j]
                                            .id + '">' + Object.values(data
                                                .brand_ditawarkan[i])[0][j]
                                            .brand_name + '</option>';
                                    }

                                    cloneBrandDitawarkan.find('select').find(
                                        'option').remove();
                                    cloneBrandDitawarkan.find('select').append(
                                        strOption);
                                }
                                ...
                                var ItemTable = {
                                    ...
                                    description_item_ditawarkan: '<textarea id="deskripsi_item_ditawarkan" name="deskripsi_umum" class="form-input col-md-12" style="min-width: 180px;" value="">' +v.description + '</textarea>'
                                };
                            ...
                            // console.log($('#penawaran_ke').val());

                            if (data.penawaran_ke > 1) {
                                $('#btn-cicil').hide();
                                // $('#cod_head').hide();
                                $('.button-cod').hide();
                                $('.percentage_dp').hide();
                                $('.metode_sisa_pembayaran').hide();
                            } else {
                                $('#btn-cicil').show();
                                // $('#cod_head').show();
                                $('.button-cod').show();
                                $('.percentage_dp').show();
                                $('.metode_sisa_pembayaran').show();
                            }
                        ...
                        $("#ppn_rekanan").val(data.ppn_rekanan);
                    ...
                    success: function(data) {
                        ...
                        if (data.result.length > 0) {
                            ...
                            for (var i = 0; i < data.result.length; i++) {
                                strHtml += '<option value="' + data.result[i].id + '" >' +
                                    data.result[i].rekanan_name + '</option>';
                            }
                        }

                        var checkIndex = parseFloat(data.indexPenawaran - data.result
                            .length);
                        var index = 1;
                        if (checkIndex <= 0) {
                        ...
                        item.append(strHtml);
                        $('#date_dibutuhkan').val(data.tanggal_butuh);
                    },
                    complete: function() {
                        waitingDialog.hide();
                    }
            ...
            $('#btn-cicil').click(function() {
                var strCod = '';
                var termin_cod = parseInt($('#termin').val());
                var dp = parseFloat($(document).find('#percentage_dp').val())
                    strCod = '<div class="form-group dp_termin"><label class="col-sm-2 control-label">DP</label><div class="col-sm-10"> <div class="input-group"><div class="input-group-addon">%</div><input type="text" class="form-control" value="' + dp + '" min="1" step="any" id="" name="" readonly> </div></div></div>'; 
                for (var i = 1; i <= termin_cod; i++) {
                ...
                $('#form-cicil').find('.dp_termin').remove();
            ...
            //   $('#penawaran_ke').change(function()
            //      {
            //       console.log($('#penawaran_ke').val());
            //           if(parseInt($('#penawaran_ke').val()) > 1 )
            //           {
            //               $('#btn-cicil').hide();
            //               $('#cod_head').hide();
            //               $('.cod_body').hide();
