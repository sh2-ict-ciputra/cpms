...
<div class="panel panel-success">
  <div class="panel-heading"><strong>Daftar Barang Masuk Hibah</strong></div>
...
<script type="text/javascript">
  ...
  var fnCheckStatus = function(data, type, row)
  {
    ...
    if (type == 'display') {
      if(parseInt(data)>1)
      {
        retVal ="<button class='reject btn btn-success btn-xs' rel='tooltip' data-toggle='tooltip' data-placement='left' title='Click to UnApprove'>UnApprove</button>";
      }
    }
    return retVal;
  }
  ...
  $(document).ready(function(){
    gentable = $('#table_data').DataTable(
    {
      //scrollY:        "300px",
        ...
        //paging:         false,
          processing: true,
          ajax: "{{ url('/inventory/hod_inventory/getDaftarBarangMasukHibah') }}",
          columns:[
      ...
      sbody.on('click','.approve',function(){
          ...
          $.ajax({
            ...
            var data = gentable.row($(this).parents('tr')).data();
            var _datasend ={'id':data.id,_token:$('input[name=_token]').val()};
            var _url = "{!! url('/inventory/hod_inventory/barangmasuk_hibah/unapprove') !!}"
            $.ajax({
                type: 'POST',
                url: _url,
                data: _datasend,
